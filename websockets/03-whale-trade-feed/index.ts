 ...
 */
import { connect, hexToUsdc, short, type Frame } from "../../shared/radion-ws";
...
function render() {
  console.clear();
  console.log(`🐋 Whale trades ≥ $${minUsd.toLocaleString()}  (top ${TOP_N})\n`);
  if (board.length === 0) {
...
connect({
  subscriptions: [{ id: "whales", channel: "large_trades", filters: { min_usd: minUsd } }],
  ...
  onEvent: (d) => {
    ...
    board.push({ usd, token: d.tokenId, taker: d.taker, at: new Date().toISOString().slice(11, 19) });
    if (board.length > 200) board.shift();
    render();
  },
});
